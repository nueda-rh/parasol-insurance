...
NOTE: GitOpsとは、インフラストラクチャとアプリケーションのデプロイおよび管理において、GitリポジトリをSingle source of truth（唯一の信頼できる情報源）として利用する運用手法です。このアプローチでは、Gitリポジトリにインフラやアプリケーションの設定・状態を定義したファイルを配置し、それを基に自動的に環境を構築・維持します。

OpenShift環境でGitOpsを使ったアプリケーションのデプロイを行うため、まずArgoCD インスタンスをあなたのNamespaceにデプロイします。 +
...
== GitOps でアプリケーションをデプロイ

{argocd} があなたのNamespaceにデプロイされたので、これを使用して GitOps でアプリケーションをデプロイできます。
...
spec:
  ...
  source:
    ...
    repoURL: https://github.com/team-ohc-jp-place/parasol-insurance.git
    targetRevision: translation-jp
  syncPolicy:
    automated:
      prune: true
      selfHeal: false
