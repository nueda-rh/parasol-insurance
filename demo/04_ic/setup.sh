...
oc apply -f git.yaml -n ${USER}
while true; do oc get statefulset/git -n ${USER} 2>&1 | grep "not found" 1>/dev/null 2>&1; if [ $? -eq 0 ]; then echo "statefulset/git does not exist yet. waiting..."; sleep 3; continue; else break; fi; done
oc wait --for jsonpath='{.status.availableReplicas}'=1 --timeout 5m statefulset/git -n ${USER}
...
oc exec pods/my-workbench-0 -c my-workbench -n ${USER} -- git clone http://git/parasol-insurance
